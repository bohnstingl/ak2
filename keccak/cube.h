...
public:
  ...
  {
  public:
    cubeIterator(uint64_t start, Cube &cube) :
        i_(start), t_(0), diff_(0), var_index_(0), cube_(cube)
    {
    ...
    bool operator==(const cubeIterator& rhs)
    {
      return rhs.i_ == i_;
    }
    bool operator!=(const cubeIterator& rhs)
  ...
  pair<Cube::cubeIterator, Cube::cubeIterator> getEqualCubeRange(uint i, uint n)
  {
    ...
    return pair<Cube::cubeIterator, Cube::cubeIterator>(
        cubeIterator(i * step, *this), cubeIterator((i + 1) * step, *this));
  };

  void sumRange(uint64_t result[], uint64_t key[],
      pair<Cube::cubeIterator, Cube::cubeIterator> range);
